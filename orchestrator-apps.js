...
function initializeActuatorDevices(packetForwarderIps) {
    // send request to all PFs to stop evaluation
    const data = {
        "start": true
    };
    ...
    switch (deviceDistributionMode) {
        ...
        case 'random':
            if(packetForwarderIps.length === 5) { //omc mode
                return [483,83,449,393,69,109,267,387,62,100,252,137,434,186,463];
            } else {
                return [388,190,288,139,337,249,142,367,2,125,191,43];
            }
    }
}
...
function getActuatorRequirement(executingGatewayIp, deviceDistributionMode) {
    switch (deviceDistributionMode) {
        ...
        case 'random':
            if(packetForwarderIps.length === 5) { //omc mode
                return [43,36,33,31,27,0,42,17,16,12];
            } else {
                return [16,14,37,12,34,15,18,8];
            }
    }
}
